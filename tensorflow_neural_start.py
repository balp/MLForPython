...
## input > hidden layer 1 (Activation func)> weights > hidden l 2
## (activation function) > weights > output layer
## compare output with intended output > cost function (cross entropy)
## optimizations functions (optimizer) > minimize cort
## Backproagation
##
## feed forward + backprop = epoch


mnist = input_data.read_data_sets("c:\\tmp", one_hot=True)
        ...
        for epoch in range(hm_epochs):
            ...
            for _ in range(int(mnist.train.num_examples/batch_size)):
                ...
                _,c = sess.run([optimizer, cost], feed_dict={x: epoch_x,
                                                             y: epoch_y})
                epoch_loss += c
            print('Epoch: {} complected out of {}, loss {}'.format(epoch,
                                                                   hm_epochs,
                                                                   epoch_loss))
